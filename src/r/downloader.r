func.download.year = function(year) {

  print(paste("Processing", year, sep=" "))

  ########################################################
  # DATA LOADING COMPONENT - ONLY RUN THIS ONCE PER YEAR #
  ########################################################
  ...
  if(year %in% c(1984:2001)) {
    ...
    if(class(first.attempt) == "try-error") {
      ...
      second.attempt = try(dbWriteTable(db , tablename , csvfile , na.strings = "" , nrow.check = rtctr , lower.case.names = TRUE) , silent = TRUE)
    }
  ...
  if(year %in% c(2002:2015)) {
    ...
    if (year >= 2012){
      ...
      fn = paste0("ftp://ftp.cdc.gov/pub/data/brfss/cdbrfs",
                  ifelse(year == 2002,
                         year,
                         substr(year, 3, 4)),
                  "asc.zip")
  ...
  #now for the given downloaded year
  ...
  save(brfss.design , file = paste0(tablename, 'design.rda'))
  ...
  for (this_table in dbListTables(db))
    dbSendQuery(db , paste("ALTER TABLE" , this_table , "SET READ ONLY"))
